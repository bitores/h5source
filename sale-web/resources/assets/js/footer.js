...
function getInfo(){
	...
	var fname = $('#txt_foot_name').val().replace(/^\s*/g,"").replace(/\s*$/g,"");
	var fcarname = $('#txt_foot_car').val().replace(/^\s*/g,"").replace(/\s*$/g,"");
	var fphone = $('#txt_foot_phone').val().replace(/^\s*/g,"").replace(/\s*$/g,"");
	var smsverify = $('#user_verify').val();
	...
	$.ajax({
		...
		success: function(d) {
			if(d.code == 0){
				$('#footer_box_message div').text('提交成功');		
				$('#footer_box_message').show();
				$('#txt_foot_car').val('');
				$('#txt_foot_phone').val('');
				$('#txt_foot_name').val('');
